...
test('stdin-filename option with stdin', async t => {
	const {stdout} = await execa(cli, ['--no-local', '--stdin', '--stdin-filename=unicorn-file'], {
		input: 'console.log()\n',
		reject: false
	});
...
test('reporter option', async t => {
	...
	try {
		await execa(cli, ['--no-local', '--reporter=compact', filepath]);
	} catch (err) {
...
test('overrides fixture', async t => {
	...
	await t.notThrows(execa(cli, ['--no-local'], {cwd}));
});
...
test.failing('ignores fixture', async t => {
	...
	await t.throws(execa(cli, ['--no-local'], {cwd}));
});

test('ignore files in .gitignore', async t => {
	const cwd = path.join(__dirname, 'fixtures/gitignore');

	const err = await t.throws(execa(cli, ['--no-local'], {cwd}));
...
test('quiet option', async t => {
	...
	const err = await t.throws(execa(cli, ['--no-local', '--quiet', '--reporter=compact', filepath]));
